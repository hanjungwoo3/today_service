...
while ($row = $result->fetch_assoc()) {
	...
	if ($ttr_assigned_date) {
		...
		if ($ttr_assigned_date) {
			...
			if ($ttr_mb_name)
				$sheet->setCellValue($left_alpabet . ($start_column + 1), date('y.n.j', strtotime($ttr_start_date)));
		}
		if ($ttr_end_date) {
...
// Rename worksheet
$objPHPExcel->getActiveSheet()->setTitle('구역배정기록(' . $tt_sdate . '_' . $tt_fdate . ')');
...
header('Content-Disposition: attachment;filename="구역배정기록(' . $tt_sdate . '_' . $tt_fdate . ').xlsx"');
