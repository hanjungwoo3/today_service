    ...
    <tr class="gubun-odd">
      ...
      <?php else: ?>
        <td rowspan="2" class="text-center" style="vertical-align: middle; min-width: 50px; width: 50px; position: relative;">
          <label class="visit-check-label <?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'disabled' : ''; ?>"
            <?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'title="이미 방문 완료된 세대입니다"' : ''; ?>>
            <input type="checkbox" class="visit-check" name="tph_visit" value="Y"
              <?= ($r['tph_visit'] == 'Y') ? 'checked="checked"' : ''; ?>       <?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'disabled="disabled"' : ''; ?>
              onclick="<?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'return false;' : 'visit_check(\'telephone\',' . $tph_id . ',this);' ?>">
        ...
        <td rowspan="2" class="text-center" style="vertical-align: middle; min-width: 50px; width: 50px; position: relative;">
          <label class="visit-check-label <?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'disabled' : ''; ?>"
            <?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'title="이미 방문 완료된 세대입니다"' : ''; ?>>
            <input type="checkbox" class="visit-check" name="tph_visit" value="N"
              <?= ($r['tph_visit'] == 'N') ? 'checked="checked"' : ''; ?>       <?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'disabled="disabled"' : ''; ?>
              onclick="<?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'return false;' : 'visit_check(\'telephone\',' . $tph_id . ',this);' ?>">
            <span class="visit-check-mark <?= ($is_absence_status && $r['tph_visit_old'] == 'Y') ? 'disabled' : ''; ?>"></span>
          </label>
        </td>
      ...
      <td style="text-align:center; vertical-align: middle; min-width: 50px; width: 50px; position: relative;" rowspan="2">
        <?php if ($condition): ?>
          <button class="btn btn-outline-info btn-sm text-center"
            onclick="condition_work('telephone', 'view', <?= $r['tph_id'] ?>, '<?= $r['tph_number'] ?>');"><i
              class="bi bi-bell"></i></button>
        <?php else: ?>
          <button class="btn btn-outline-secondary btn-sm text-center"
            onclick="condition_work('telephone', 'add', <?= $r['tph_id'] ?>, '<?= $r['tph_number'] ?>');"><i
    ...
    <tr class="gubun-even">
      <td class="text-left" colspan="2" style="word-wrap: break-word; word-break: break-word; overflow-wrap: break-word;">
        <div style="padding-right: 5px;">
          <?php if ($r['tph_address'])
            echo kakao_menu($r['tph_address']); ?>
          <a href="tel:<?= $r['tph_number'] ?>" class="btn btn-sm btn-outline-info"><i class="bi bi-telephone"></i></a>
          <button type="button" class="btn btn-sm btn-outline-info" onclick="show_tel_info(<?= $tph_id; ?>);"><i
