...
<div class="territory-view" tt_id="<?= $tt_id ?>" update_page="<?= $update_page ?>"
  ...
  <style>
    ...
    /* 체크박스 크기 강제 적용 (캐시 문제 해결용) 및 정렬 수정 */
    .visit-check-label .visit-check-mark {
    ...
    .visit-check-label .visit-check-mark:after {
      left: 11px !important;
      top: 4px !important;
    }

    /* 특이사항 버튼 여백 */
    .condition-btn-margin {
  ...
  <div class="territory-view-header">
    <div class="territory_header">
      ...
      <div class="px-3 py-2 align-middle clearfix">
        <button type="button" class="btn btn-sm btn-outline-danger align-middle"
          onclick="memo_work('territory',<?= $tt_id ?>);"><i class="bi bi-pencil"></i> 특이사항 전달</button>
        <button type="button" class="btn btn-sm btn-outline-primary mb-0 align-middle float-right"
          onclick="close_territory_view(<?= $tt_id ?>);" name="button" data-dismiss="modal"><i
        ...
        <tbody>
          <tr>
            ...
            <td>
              ...
              <?php if ($row['tt_address'])
                echo kakao_menu($row['tt_address']); ?>
            </td>
          </tr>
          <tr>
            ...
            <td>
              ...
              <?php if ($row['tt_address2'])
                echo kakao_menu($row['tt_address2']); ?>
            </td>
          </tr>
          <tr>
            <th class="bg-light text-center"><?= get_status_text($row['tt_status']) ?></th>
            <td class="text-center"><?= !empty_date($row['tt_start_date']) ? $row['tt_start_date'] : ''; ?> ~
              <?= !empty_date($row['tt_end_date']) ? $row['tt_end_date'] : ''; ?>
            </td>
          </tr>
          <tr>
    ...
    <div class="px-3 py-2 align-left clearfix border-top">
      <button type="button" class="btn btn-sm btn-outline-secondary align-middle mb-0 mr-2"
        onclick="territory_work('record',<?= $tt_id ?>,'','','');"><i class="bi bi-list-task"></i> 봉사 기록</button>
      <?php if ($row['tt_polygon']): ?>
        <button type="button" onclick="map_view('territory', <?= $tt_id ?>)"
          class="btn btn-sm btn-outline-secondary mb-0 align-middle">
    ...
    <table id="fixed-head" class="table table-sm" style="table-layout: fixed;">
      <colgroup>
        ...
        <col width="65px">
        <col>
        <col width="<?= $row['tt_type'] == '편지' ? '90px' : '45px' ?>">
