...
<div class="container-fluid">
  <form id="admin_meeting_form" method="POST">
    ...
    <div class="form-group row">
      ...
      <div class="col-8">
        <input type="time" class="form-control time-custom font-weight-bold" name="time" id="time" value="<?= $ms_time ?>"
    ...
    <div class="form-group row">
      ...
      <div class="col-8">
        <input type="time" class="form-control time-custom font-weight-bold" name="st_time" id="st_time"
          value="<?= $ms_start_time ?>" style="padding-left:1rem;" onchange="timemin();">
      </div>
    </div>
    ...
    <div class="form-group row">
      ...
      <div class="col-8">
        <input type="time" class="form-control time-custom font-weight-bold" name="fi_time" id="fi_time"
          value="<?= $ms_finish_time ?>" style="padding-left:1rem;" onchange="timemax();">
      </div>
    </div>
      ...
      <div class="col-8">
        <select class="form-control select-custom font-weight-bold" name="group" id="group">
          ...
          <?php foreach ($group as $key => $value)
            echo "<option value='" . $key . "' " . get_selected_text($key, $g_id) . ">" . $value . "</option>"; ?>
        </select>
      ...
      <div class="col-8">
        <select class="form-control select-custom font-weight-bold" name="place" id="place">
          ...
          <?php foreach ($mp as $key => $value)
            echo "<option value='" . $key . "' " . get_selected_text($key, $mp_id) . ">[" . $value['mp_name'] . "] " . $value['mp_address'] . "</option>"; ?>
        </select>
      </div>
    </div>
      ...
      <div class="col-8">
        <div class="mb-2">
          <div class="custom-control custom-radio">
            <input type="radio" id="ms_limit_default" name="ms_limit_type" class="custom-control-input" value="default"
              <?= ($ms_limit === '') ? 'checked' : '' ?>>
            <label class="custom-control-label" for="ms_limit_default">기본값 따름</label>
          </div>
          <div class="custom-control custom-radio">
            <input type="radio" id="ms_limit_custom" name="ms_limit_type" class="custom-control-input" value="custom"
              <?= ($ms_limit !== '') ? 'checked' : '' ?>>
            <label class="custom-control-label" for="ms_limit_custom">직접 설정</label>
          </div>
        </div>

        <div class="input-group" id="ms_limit_input" style="display: <?= ($ms_limit !== '') ? 'flex' : 'none' ?>;">
          <input type="number" class="form-control font-weight-bold" name="ms_limit_display" id="ms_limit_display"
            value="<?= ($ms_limit !== '' && $ms_limit != '0') ? $ms_limit : '' ?>" min="1">
          <input type="hidden" name="ms_limit" id="ms_limit" value="<?= ($ms_limit !== '') ? $ms_limit : '' ?>">
