...
/**
 * 엑셀 날짜를 PHP 날짜 형식으로 변환
 * 
 * @param string $date 엑셀 날짜 값
 * @return string PHP 날짜 형식 (Y-m-d)
 */
function time_convert_EXCEL_to_PHP($date) {
	$return = '';
	...
	if($date){
	  if(preg_match("/^([0-9]{4})-([0-9]{2})-([0-9]{2})$/", $date)){
		...
		$return = ($date- 25569) * 86400-60*60*9;
...
/**
 * 엑셀 데이터 업데이트를 위한 SQL 케이스문 생성
 * 
 * @param array $arr_mb_id 회원 ID 배열
 * @param string $gubun 구분자
 * @param string $field 필드명
 * @param array $arr_field_item 필드값 배열
 * @return string SQL 케이스문
 */
function excel_update($arr_mb_id, $gubun, $field, $arr_field_item){

	global $mysqli;
	
	$sql = "$gubun$field = \r\n";
	$sql .= "        case \r\n";
	for ($num = 0; $num < count($arr_mb_id); $num++){
		// SQL 인젝션 방지를 위한 이스케이프 처리
		$escaped_id = $mysqli->real_escape_string($arr_mb_id[$num]);
		$escaped_item = $mysqli->real_escape_string($arr_field_item[$num]);
		$sql .= "             when mb_id = '{$escaped_id}' then '{$escaped_item}'\r\n";
	}
	$sql .= "        end\r\n";
...
if (version_compare(PHP_VERSION, '8.2.0', '>=')) {
	require_once 'member_excel_upload_php8.php';
	exit;
}
...
try {
  ...
  for ($i = 2 ; $i <= $maxRow ; $i++) {
	  ...
	  $mb_id				= $objWorksheet->getCell('B' . $i)->getValue(); // B열
	  $mb_name			= $objWorksheet->getCell('C' . $i)->getValue(); // C열
  	$mb_pw				= $objWorksheet->getCell('D' . $i)->getValue(); // D열
	  $mb_hp				= $objWorksheet->getCell('E' . $i)->getValue(); // E열
	  $mb_sex				= $objWorksheet->getCell('F' . $i)->getValue(); // F열
  	$mb_address			= $objWorksheet->getCell('G' . $i)->getValue(); // G열
	  $mb_position		= $objWorksheet->getCell('H' . $i)->getValue(); // H열
	  $mb_pioneer			= $objWorksheet->getCell('I' . $i)->getValue(); // I열
  	$mb_display			= $objWorksheet->getCell('J' . $i)->getValue(); // J열
  	$g_id				= $objWorksheet->getCell('K' . $i)->getValue(); // K열
  	$mb_movein_date 	= time_convert_EXCEL_to_PHP($objWorksheet->getCell('L' . $i)->getValue()); // L열
  	$mb_moveout_date 	= time_convert_EXCEL_to_PHP($objWorksheet->getCell('M' . $i)->getValue()); // M열
	...
	if ($strUPD){
  	...
  	if($str_mb_pw) $arr_mb_pw = explode(",",$str_mb_pw);
  	...
  	if($arr_mb_pw) $sql .= excel_update($arr_mb_id, ",", "mb_hash", $arr_mb_pw);
