...
while ($row = $result->fetch_assoc()) {
	...
	if ($tpr_assigned_date) {
		...
		// 시작날짜, 마친날짜 입력
		if ($tpr_assigned_date) {
			$activeWorksheet->setCellValue($left_alpabet . ($start_column + 1), date('y.m.d', strtotime($tpr_assigned_date))); // 시작날짜
		} else {
			if ($tpr_mb_name)
				$activeWorksheet->setCellValue($left_alpabet . ($start_column + 1), date('y.m.d', strtotime($tpr_start_date)));
		}
		if ($tpr_end_date) {
			$activeWorksheet->setCellValue($right_alpabet . ($start_column + 1), date('y.m.d', strtotime($tpr_end_date))); // 마친날짜
...
while ($row = $result->fetch_assoc()) {
	...
	if ($tp_assigned_date) {
		...
		// 시작날짜, 마친날짜 입력
		if ($tp_assigned_date) {
			$activeWorksheet->setCellValue($left_alpabet . ($start_column + 1), date('y.m.d', strtotime($tp_assigned_date))); // 시작날짜
		} else {
			if ($row['mb_id']) {
				if ($row['tp_mb_date'] && !empty_date($row['tp_mb_date'])) {
					$activeWorksheet->setCellValue($left_alpabet . ($start_column + 1), date('y.m.d', strtotime($row['tp_mb_date'])));
				} else {
					$activeWorksheet->setCellValue($left_alpabet . ($start_column + 1), date('y.m.d', strtotime($tp_start_date)));
				}
			}
		}
		if ($tp_end_date) {
			$activeWorksheet->setCellValue($right_alpabet . ($start_column + 1), date('y.m.d', strtotime($tp_end_date))); // 마친날짜
...
//엑셀 파일 다운로드를 위한 설정
$excelFileName = iconv('UTF-8', 'EUC-KR', '전화구역배정기록(' . $tp_sdate . '_' . $tp_fdate . ').xlsx');
