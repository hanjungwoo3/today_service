...
<div class="container-fluid">
  ...
  <form id="territory_house_form">
      ...
      <colgroup>
        ...
        <thead class="thead-light">
          <tr>
            ...
            <th class="text-center">삭제 &nbsp;<input type="checkbox" class="align-middle" onclick="if($(this).is(':checked')){ $('#territory_house_form tbody input[type=checkbox]').prop('checked', true); }else{ $('#territory_house_form input[type=checkbox]').prop('checked', false); }"></th>
            <th class="text-center"></th>
          </tr>
        </thead>
        <tbody>
        <?php while ($row = $result->fetch_assoc()):?>
          ...
          <tr>
            ...
            <td><input type="text" value="<?=$row['h_address1']?>" class="form-control" name="territory_house[<?=$h_id;?>][h_address1]"></td>
            <td><input type="text" value="<?=$row['h_address2']?>" class="form-control" name="territory_house[<?=$h_id;?>][h_address2]"></td>
            <td><input type="text" value="<?=$row['h_address3']?>" class="form-control" name="territory_house[<?=$h_id;?>][h_address3]"></td>
            <td><input type="text" value="<?=$row['h_address4']?>" class="form-control" name="territory_house[<?=$h_id;?>][h_address4]"></td>
            <td><input type="text" value="<?=$row['h_address5']?>" class="form-control" name="territory_house[<?=$h_id;?>][h_address5]"></td>
            <td class="text-center align-middle">
              <span class="condition-chip<?=$row['h_condition']?>" ><?=get_house_condition_text($row['h_condition'])?></span>
            </td>
            <td class="text-center align-middle"><input type="checkbox" class="align-middle" name="territory_house[<?=$h_id;?>][delete]" value="delete"></td>
            <td class="text-center dragHandle align-middle"><i class="bi bi-grip-vertical"></i></td>
          </tr>
        <?php endwhile;?>
        </tbody>
    ...
    <div class="mt-4">
      <button type="button" class="btn btn-outline-primary float-left" onclick="territory_house_add();"><i class="bi bi-plus-circle-dotted"></i> 추가</button>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    // input 필드 포커스 시
    $('#territory_house_form input[type="text"]').on('focus', function() {
      // 모달을 완전히 고정
      $('#popup-modal').css({
        'position': 'fixed',
        'top': '0',
    ...
    // input 필드 블러 시
    $('#territory_house_form input[type="text"]').on('blur', function() {
      // 키보드가 닫힐 때까지 잠시 대기
      setTimeout(function() {
        // 모달 스타일을 원래대로 복원
        $('#popup-modal .modal-dialog').css({
