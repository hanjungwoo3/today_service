...
function get_house_condition_text($condition){
	...
	switch ($condition) {
		case '1':$return = '재방'; break;
		case '2':$return = '연구'; break;
		case '3':$return = !empty($c_house_condition[3])?$c_house_condition[3]:'JW'; break;
		case '4':$return = !empty($c_house_condition[4])?$c_house_condition[4]:'없는집'; break;
		case '5':$return = !empty($c_house_condition[5])?$c_house_condition[5]:'수정요청'; break;
		case '6':$return = !empty($c_house_condition[6])?$c_house_condition[6]:'심한반대'; break;
		case '7':$return = !empty($c_house_condition[7])?$c_house_condition[7]:'외국인'; break;
		case '8':$return = !empty($c_house_condition[8])?$c_house_condition[8]:'기타'; break;
		case '9':$return = !empty($c_house_condition[9])?$c_house_condition[9]:'별도구역'; break;
		case '10':$return = !empty($c_house_condition[10])?$c_house_condition[10]:'추가'; break;
...
function get_member_position_text($position){
	switch ($position) {
		case '1':$return = '봉사의 종'; break;
		case '2':$return = '장로'; break;
		case '3':$return = '순회감독자'; break;
		default:$return = '';break;
	}
...
function get_member_pioneer_text($pioneer){
	switch ($pioneer) {
		case '1':$return = '전도인'; break;
		case '2':$return = '정규'; break;
		case '3':$return = '특별'; break;
		case '4':$return = '선교인'; break;
		default:$return = '';break;
	}
...
function get_week_number_text($week){
	switch ($week) {
		case '1':$return = '첫째 주'; break;
		case '2':$return = '둘째 주'; break;
		case '3':$return = '셋째 주'; break;
		case '4':$return = '넷째 주'; break;
		case '5':$return = '다섯째 주'; break;
		case '6':$return = '마지막 주'; break;
		default:$return = '';break;
	}
...
function get_percent($range, $total){
	$return = 0;
	if($range != 0 && $total != 0) $return = round(($range/$total)*100, 2);
...
function get_group_data_all(){
	...
	$result = $mysqli->query($sql);
	if($result->num_rows > 0){
		while ($row = $result->fetch_assoc()) $return[$row['g_id']] = $row['g_name'];
	}
...
function get_group_member_data($g_id){
	...
	$result = $mysqli->query($sql);
	if($result->num_rows > 0){
		while ($row = $result->fetch_assoc()) $return[] = $row['mb_id'];
	}
...
function get_member_names($mb_ids){
  ...
  $result = $mysqli->query($sql);
	if($result->num_rows > 0){
		while ($row = $result->fetch_assoc()) $return[] = $row['mb_name'];
	}
...
function is_moveout($mb_id){
	...
	if($result->num_rows > 0){
		$row = $result->fetch_assoc();
		if($row['mb_moveout_date'] && $row['mb_moveout_date'] != '0000-00-00') $return = true;
	}
...
function get_member_board_auth($mb_id){
	$auth = array(1,7);
	if(get_member_auth($mb_id) == 1 || get_member_pioneer($mb_id) > 1) $auth[] = 2;	//파이오니아
	if(get_member_auth($mb_id) == 1 || is_guide($mb_id)) $auth[] = 3; //인도자
	if(get_member_auth($mb_id) == 1 || get_member_position($mb_id) >= 1) $auth[] = 4; //봉사의 종
	if(get_member_auth($mb_id) == 1 || get_member_position($mb_id) >= 2) $auth[] = 5; //장로
	if(is_admin($mb_id)) $auth[] = 6;  //관리자
...
function is_admin($mb_id){
	...
	$return = false;
	if(get_member_auth($mb_id) == '1' || get_member_auth($mb_id) == '2') $return = true;
...
function is_guide($mb_id){
	...
	if($row['s1'] > 0 || $row['s2'] > 0) $return = true;
...
function get_ms_id_by_guide($mb_id){
	...
	$result = $mysqli->query($sql);
	if($result->num_rows > 0) while ($row = $result->fetch_assoc()) $return[] = $row['ms_id'];
...
function getNthWeekday($year, $month, $week, $weekday){ // 주의 지정이 옳은지 확인함

  if( $week < 1 || $week > 5 ) return false;
  if( $weekday < 0 || $weekday > 6 ) return false; // 요일의 지정이 옳은지 확인함
  ...
  if( !checkdate($month, $resultDay, $year) ) return false; // 마지막으로 처리 결과가 올바른 날짜인지 확인함.
		...
		while($ma = $ma_result->fetch_assoc()){
			...
			if($ma['ma_auto'] == 1){
				...
				if($day !== false){
					...
					if($date == $s_date) $ma_array[] = $ma['ma_id'];
				}
			}else{
				$ma_array[] = $ma['ma_id'];
			}
	...
	if($ma_auto == 1){
		...
		if ($ma_date == '0000-00-00 00:00:00' && $ma_date2 == '0000-00-00 00:00:00') {
			...
			if($month1 == $month2){
				if($time1 && $time2){
					$return = ($time1 == $time2)?$month1.' '.$time1:$month1.' '.$time1.' ~ '.$time2;
				}elseif(empty($time1) && empty($time2)) $return = $month1;
			}else{
				$return = (!empty($time1 or $time2))?$month1.' '.$time1.' ~ '.$month2.' '.$time2:$month1.' ~ '.$month2;
			}
		}
...
function get_copy_ms_id($ms_ids){
	...
	$result = $mysqli->query($sql);
	if($result->num_rows > 0) while ($row = $result->fetch_assoc()) $return[] = $row['copy_ms_id'];
	$return = implode(',', array_unique($return));
...
function get_meeting_place_data_all(){
	...
	$result = $mysqli->query($sql);
	if($result->num_rows > 0){
		while ($row = $result->fetch_assoc()) $return[$row['mp_id']] = array('mp_name' => $row['mp_name'], 'mp_address' => $row['mp_address']);
	}
...
function get_meeting_id($meeting_date, $ms_id){
	...
	if($result->num_rows > 0){
		...
		$sql = "INSERT INTO ".MEETING_TABLE." (mb_id, m_cancle, m_cancle_reason, m_contents, m_guide, m_date, ms_id, ms_time, ms_week, ms_type, ms_guide, ms_guide2, g_id, mp_id, mp_name, mp_address, m_start_time, m_finish_time)
...
function get_assigned_member_of_meeting($m_id){
	...
	if($result2->num_rows > 0){
	  while ($row2 = $result2->fetch_assoc()) {
	    ...
	    foreach ($tt_assigned as $key => $member_id) $assigned_ministers[] = $member_id; // 배정된 전도인에 해당 전도인 추가
	  }
	}
	...
	if($result2->num_rows > 0){
	  while ($row2 = $result2->fetch_assoc()) {
	    ...
	    foreach ($tp_assigned as $key => $member_id) $assigned_ministers[] = $member_id; // 배정된 전도인에 해당 전도인 추가
	  }
	}
	...
	if($result2->num_rows > 0){
	  while ($row2 = $result2->fetch_assoc()) {
	    ...
	    foreach ($d_assigned as $key => $member_id) $assigned_ministers[] = $member_id; // 배정된 전도인에 해당 전도인 추가
		}
	}
...
function get_assigned_group($assigned_members,$assigned_group){
	...
	if(empty($assigned_group)){
		...
		if(count($assigned_group) == 1){
			$arr = array();
			while($assigned_members) $arr[] = array_splice($assigned_members, 0, $assigned_group[0]);
			return $arr;
		}else{
			$arr = array();
			foreach($assigned_group as $group) $arr[] = array_splice($assigned_members, 0, $group);

			if($assigned_members){
				foreach($assigned_members as $member) $arr[] = $member;
			}
			return $arr;
		}
	}
...
function get_assigned_group_name($assigned_members,$assigned_group){
	...
	if(empty($assigned_group)){
		...
		if(count($assigned_group) == 1){
			$arr = array();
			while($assigned_members) $arr[] = get_assigned_member_name(implode(',', array_splice($assigned_members, 0, $assigned_group[0])));
			return $arr;
		}else{
			$arr = array();
			foreach($assigned_group as $group) $arr[] = get_assigned_member_name(implode(',', array_splice($assigned_members, 0, $group)));

			if($assigned_members){
				foreach($assigned_members as $member) $arr[] = get_member_name($member);
			}
			return $arr;
		}
	}
	...
	foreach ($arr as $key => $value) {
		if(!empty($value)){
			...
			if($result->num_rows > 0){
				...
				if($member['success']  == 0) unset($arr[$key]);
			}
		}
	}
	...
	<div class="collapse navbar-collapse" id="navbarNavAltMarkup">
		<div class="navbar-nav">
			...
			if($active_menu == 'minister'): // 봉사자
				...
				if(RETURNVISIT_USE == 'use'): $page_arr[] = array('재방문', BASE_PATH.'/pages/minister_returnvisit.php'); endif;
				...
				if(MINISTER_STATISTICS_USE == 'use'): $page_arr[] = array('나의 통계', BASE_PATH.'/pages/minister_statistics.php'); endif;
				if(!is_moveout(mb_id())): $page_arr[] = array('회중 봉사', BASE_PATH.'/pages/minister_meeting_schedule.php'); endif;
			...
			elseif($active_menu == 'board'):  // 공지
				...
				$board_arr[] = 1;
				$board_arr[] = 7;
				if(in_array(2, get_member_board_auth(mb_id()))): $board_arr[] = 2; endif;
				if(in_array(3, get_member_board_auth(mb_id()))): $board_arr[] = 3; endif;
				if(in_array(4, get_member_board_auth(mb_id()))): $board_arr[] = 4; endif;
				if(in_array(5, get_member_board_auth(mb_id()))): $board_arr[] = 5; endif;
				if(in_array(6, get_member_board_auth(mb_id()))): $board_arr[] = 6; endif;
				foreach (array_filter($board_arr) as $value) echo '<a class="nav-item nav-link '.get_active_text($active_page, $value).'" href="'.BASE_PATH.'/pages/board.php?auth='.$value.'">'.$board_title[$value].get_current_text($active_page, $value).'</a>';
			elseif($active_menu == 'guide'): // 인도자
				...
				if(GUIDE_STATISTICS_USE == 'use'): $page_arr[] = array('통계', BASE_PATH.'/pages/guide_statistics.php'); endif;
			elseif($active_menu == 'admin'): // 관리자
				...
				if(get_member_auth(mb_id()) == 1): $page_arr[] = array('설정', BASE_PATH.'/pages/admin_setting.php'); endif;
			endif;
			foreach (array_filter($page_arr) as $value) echo '<a class="nav-item nav-link '.get_active_text($active_page, $value[0]).'" href="'.$value[1].'">'.$value[0].get_current_text($active_page, $value[0]).'</a>';
			?>
		</div>
	</div>
	<?php
...
function footer_menu($active_page){
	...
	if(DISPLAY_USE == 'use') $navclass++;
	if(is_guide(mb_id()) || is_admin(mb_id())) $navclass++;
	if(is_admin(mb_id())) $navclass++;
	if(is_moveout(mb_id())) $navclass = 2;
						...
						<g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
							<g id="Artboard-Copy" transform="translate(-15.000000, -703.000000)">
								<g id="001_off" transform="translate(15.000000, 703.000000)">
									...
									<g id="Group" transform="translate(3.000000, 4.000000)" stroke="#111111" stroke-linejoin="round" stroke-width="1.5">
										...
										<polyline id="Path-3" stroke-linecap="round" points="4 12.4977436 4 24.4977436 22 24.4977436 22 12.4977436"></polyline>
							...
							<g id="Artboard-Copy" transform="translate(-75.000000, -703.000000)">
								<g id="icon" transform="translate(15.000000, 703.000000)">
									<g id="Group-3" transform="translate(60.000000, 0.000000)">
										...
										<g id="Group" transform="translate(4.000000, 5.000000)" stroke="#111111" stroke-linejoin="round" stroke-width="1.5">
											<g id="Group-2">
												<polyline id="Path-4" transform="translate(12.000000, 13.000000) scale(1, -1) translate(-12.000000, -13.000000) " points="10 15 10 11 14 11 14 15"></polyline>
												<polygon id="Path-3" stroke-linecap="round" points="0 3 0 21 24 21 24 3"></polygon>
												<polyline id="Path" stroke-linecap="round" transform="translate(12.000000, 9.000000) scale(1, -1) translate(-12.000000, -9.000000) " points="3 12 12 6 21 12"></polyline>
								...
								<g id="icon" transform="translate(15.000000, 703.000000)">
									<g id="Group-3" transform="translate(60.000000, 0.000000)">
										...
										<g id="Group-2" transform="translate(4.000000, 5.000000)">
											<path d="M2,21.0139971 L24,21.0139971 L24,5.01399714 L4,5.01399714 C2.8954305,5.01399714 2,5.90942764 2,7.01399714 L2,21.0139971 L2,21.0139971 Z" id="Path-6" fill-opacity="0.2" fill="#C9DEFF"></path>
											<polyline id="Path-4" stroke="#4A6DA7" stroke-width="1.5" stroke-linejoin="round" transform="translate(12.000000, 13.000000) scale(1, -1) translate(-12.000000, -13.000000) " points="10 15 10 11 14 11 14 15"></polyline>
											...
											<polyline id="Path" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(12.000000, 9.000000) scale(1, -1) translate(-12.000000, -9.000000) " points="3 12 12 6 21 12"></polyline>
								...
								<g id="Artboard-Copy" transform="translate(-135.000000, -703.000000)">
									<g id="icon" transform="translate(15.000000, 703.000000)">
										<g id="Group-3" transform="translate(120.000000, 0.000000)">
											<rect id="Rectangle-Copy" fill-opacity="0" fill="#FFFFFF" x="0" y="0" width="32" height="32"></rect>
											<g id="Group" transform="translate(7.000000, 2.000000)" stroke="#111111" stroke-linejoin="round" stroke-width="1.5">
												<g id="Group-2">
													<path d="M1,28 L1,26.5 C1,25.6715729 1.67157288,25 2.5,25 C3.32842712,25 4,25.6715729 4,26.5 L4,28 L4,28" id="Path-4" transform="translate(2.500000, 26.500000) scale(1, -1) translate(-2.500000, -26.500000) "></path>
													<path d="M14,28 L14,26.5 C14,25.6715729 14.6715729,25 15.5,25 C16.3284271,25 17,25.6715729 17,26.5 L17,28 L17,28" id="Path-4-Copy" transform="translate(15.500000, 26.500000) scale(1, -1) translate(-15.500000, -26.500000) "></path>
									...
									<g id="icon" transform="translate(15.000000, 703.000000)">
										<g id="Group-3" transform="translate(120.000000, 0.000000)">
											...
											<g id="Group-2" transform="translate(7.000000, 2.000000)">
												<path d="M2.3,25 L17.3,25 L17.3,7 L4.3,7 C3.1954305,7 2.3,7.8954305 2.3,9 L2.3,25 L2.3,25 Z" id="Path-6" fill-opacity="0.2" fill="#C9DEFF"></path>
												<path d="M1,28 L1,26.5 C1,25.6715729 1.67157288,25 2.5,25 C3.32842712,25 4,25.6715729 4,26.5 L4,28 L4,28" id="Path-4" stroke="#4A6DA7" stroke-width="1.5" stroke-linejoin="round" transform="translate(2.500000, 26.500000) scale(1, -1) translate(-2.500000, -26.500000) "></path>
												<path d="M14,28 L14,26.5 C14,25.6715729 14.6715729,25 15.5,25 C16.3284271,25 17,25.6715729 17,26.5 L17,28 L17,28" id="Path-4-Copy" stroke="#4A6DA7" stroke-width="1.5" stroke-linejoin="round" transform="translate(15.500000, 26.500000) scale(1, -1) translate(-15.500000, -26.500000) "></path>
												...
												<polyline id="Path-4-Copy-2" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" points="3 18.6363636 3 14.6363636 15 14.6363636 15 18.6363636"></polyline>
							...
							<g id="Artboard-Copy" transform="translate(-195.000000, -703.000000)">
								<g id="icon" transform="translate(15.000000, 703.000000)">
									<g id="004_off" transform="translate(180.000000, 0.000000)">
										<rect id="Rectangle" fill-opacity="0" fill="#FFFFFF" x="0" y="0" width="32" height="32"></rect>
										<g id="Group" transform="translate(6.000000, 6.000000)" stroke="#111111" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5">
											<polyline id="Path-3" transform="translate(1.000000, 10.000000) rotate(90.000000) translate(-1.000000, -10.000000) " points="-2 9 -2 11 4 11 4 9"></polyline>
											<line x1="20" y1="9" x2="20" y2="11" id="Path-3-Copy-2" transform="translate(20.000000, 10.000000) rotate(90.000000) translate(-20.000000, -10.000000) "></line>
											<line x1="18.5" y1="4.5" x2="19.5" y2="6.5" id="Path-3-Copy-3" transform="translate(19.000000, 5.500000) rotate(90.000000) translate(-19.000000, -5.500000) "></line>
											<line x1="18.5" y1="13.5" x2="19.5" y2="15.5" id="Path-3-Copy-4" transform="translate(19.000000, 14.500000) scale(-1, 1) rotate(90.000000) translate(-19.000000, -14.500000) "></line>
											<polyline id="Path-3-Copy" transform="translate(8.500000, 10.000000) rotate(90.000000) translate(-8.500000, -10.000000) " points="15.5975395 4.5 -1.5 4.5 3.5 15.5 13.5 15.5 18.5 4.5"></polyline>
										</g>
									</g>
								</g>
							</g>
								...
								<g id="icon" transform="translate(15.000000, 703.000000)">
									<g id="004_on" transform="translate(180.000000, 0.000000)">
										...
										<g id="Group" transform="translate(6.000000, 6.000000)">
											<path d="M3,14.6363636 L14,20 L14,2.15463772 L4.19377532,6.47415692 C3.46826423,6.79373547 3,7.51168002 3,8.30445792 L3,14.6363636 L3,14.6363636 Z" id="Path-6" fill-opacity="0.2" fill="#C9DEFF"></path>
											<polyline id="Path-3" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(1.000000, 10.000000) rotate(90.000000) translate(-1.000000, -10.000000) " points="-2 9 -2 11 4 11 4 9"></polyline>
											<line x1="20" y1="9" x2="20" y2="11" id="Path-3-Copy-2" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(20.000000, 10.000000) rotate(90.000000) translate(-20.000000, -10.000000) "></line>
											<line x1="18.5" y1="4.5" x2="19.5" y2="6.5" id="Path-3-Copy-3" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(19.000000, 5.500000) rotate(90.000000) translate(-19.000000, -5.500000) "></line>
											<line x1="18.5" y1="13.5" x2="19.5" y2="15.5" id="Path-3-Copy-4" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(19.000000, 14.500000) scale(-1, 1) rotate(90.000000) translate(-19.000000, -14.500000) "></line>
											<polyline id="Path-3-Copy" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(8.500000, 10.000000) rotate(90.000000) translate(-8.500000, -10.000000) " points="15.5975395 4.5 -1.5 4.5 3.5 15.5 13.5 15.5 18.5 4.5"></polyline>
										</g>
									</g>
								</g>
							</g>
							...
							<g id="Artboard-Copy" transform="translate(-255.000000, -703.000000)">
								<g id="icon" transform="translate(15.000000, 703.000000)">
									<g id="005_off" transform="translate(240.000000, 0.000000)">
										...
										<path d="M6.97799789,28 L6,21.7190187 L13.1142801,18.0681078 L13.1142801,15.4132012 L10.7637448,12.122038 L10.7637448,7.74091542 C11.8884394,5.24697181 13.486547,4 15.5580674,4 C18.6653481,4 20.2302418,7.74091542 20.2302418,7.74091542 C20.2302418,7.74091542 20.2302418,9.20128961 20.2302418,12.122038 L17.8131939,15.4132012 L17.8131939,18.0681078 L26,21.7190187 L25.0220021,28" id="Path-5" stroke="#111111" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
										<polyline id="Path-3-Copy-4" stroke="#111111" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(14.570231, 24.184230) scale(-1, 1) rotate(90.000000) translate(-14.570231, -24.184230) " points="10.7544609 22.7544609 12.9258856 25.6139997 18.3860003 25.6139997"></polyline>
										<line x1="16.3997042" y1="18.9955503" x2="19.4395895" y2="22.4955503" id="Path-3-Copy-5" stroke="#111111" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(17.697627, 20.797923) rotate(90.000000) translate(-17.697627, -20.797923) "></line>
									</g>
								</g>
							</g>
						</g>
							...
							<g id="Artboard-Copy-2" transform="translate(-255.000000, -703.000000)">
								<g id="icon" transform="translate(15.000000, 703.000000)">
									<g id="005_on" transform="translate(240.000000, 0.000000)">
										<rect id="Rectangle-Copy-4" fill-opacity="0" fill="#FFFFFF" x="0" y="0" width="32" height="32"></rect>
										<path d="M7,28 L25,28 L26,21.8495492 L19.9508565,19.5 L16,22.7187021 L13.1404612,20.4195466 L9.05378133,22.3014939 C8.48330209,22.5642039 8.06936348,23.0796362 7.93597773,23.6933717 L7,28 L7,28 Z" id="Path-6" fill-opacity="0.2" fill="#C9DEFF"></path>
										<path d="M6.97799789,28 L6,21.7190187 L13.1142801,18.0681078 L13.1142801,15.4132012 L10.7637448,12.122038 L10.7637448,7.74091542 C11.8884394,5.24697181 13.486547,4 15.5580674,4 C18.6653481,4 20.2302418,7.74091542 20.2302418,7.74091542 C20.2302418,7.74091542 20.2302418,9.20128961 20.2302418,12.122038 L17.8131939,15.4132012 L17.8131939,18.0681078 L26,21.7190187 L25.0220021,28" id="Path-5" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path>
										<polyline id="Path-3-Copy-4" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(14.570231, 24.184230) scale(-1, 1) rotate(90.000000) translate(-14.570231, -24.184230) " points="10.7544609 22.7544609 12.9258856 25.6139997 18.3860003 25.6139997"></polyline>
										<line x1="16.3997042" y1="18.9955503" x2="19.4395895" y2="22.4955503" id="Path-3-Copy-5" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" transform="translate(17.697627, 20.797923) rotate(90.000000) translate(-17.697627, -20.797923) "></line>
									</g>
								</g>
							</g>
						</g>
							...
							<g id="Artboard-Copy" transform="translate(-315.000000, -703.000000)">
								<g id="icon" transform="translate(15.000000, 703.000000)">
									<g id="006_off" transform="translate(300.000000, 0.000000)">
										...
										<g id="Group-4" transform="translate(5.000000, 5.000000)" stroke="#111111" stroke-width="1.5">
											...
											<path d="M11.8384558,14.9119815 C12.2356664,14.8272595 12.6110434,14.6835728 12.9549863,14.4905222 C14.1753689,13.8055374 15,12.4990702 15,11 C15,8.790861 13.209139,7 11,7 C8.790861,7 7,8.790861 7,11 C7,11.5994585 7.13186637,12.168118 7.3681741,12.6785534 C7.7254563,13.4502992 8.32148632,14.0889476 9.06147749,14.499712" id="Oval" stroke-linecap="round" transform="translate(11.000000, 10.955991) rotate(30.000000) translate(-11.000000, -10.955991) "></path>
										</g>
									</g>
								</g>
							</g>
								...
								<g id="icon" transform="translate(15.000000, 703.000000)">
									<g id="006_on" transform="translate(300.000000, 0.000000)">
										...
										<g id="Group-4" transform="translate(5.000000, 5.000000)">
											...
											<path d="M11.8384558,14.9119815 C12.2356664,14.8272595 12.6110434,14.6835728 12.9549863,14.4905222 C14.1753689,13.8055374 15,12.4990702 15,11 C15,8.790861 13.209139,7 11,7 C8.790861,7 7,8.790861 7,11 C7,11.5994585 7.13186637,12.168118 7.3681741,12.6785534 C7.7254563,13.4502992 8.32148632,14.0889476 9.06147749,14.499712" id="Oval" stroke="#4A6DA7" stroke-width="1.5" stroke-linecap="round" transform="translate(11.000000, 10.955991) rotate(30.000000) translate(-11.000000, -10.955991) "></path>
										</g>
									</g>
								</g>
							</g>
...
function kakao_menu($address){
	ob_start();?>
	<button type="button" class="btn btn-outline-info btn-sm ml-1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		<i class="bi bi-geo-alt"></i>
	</button>
	<div class="dropdown-menu">
		<button class="dropdown-item" type="button" onclick="daum_roadview('<?=DEFAULT_ADDRESS.' '.$address?>')">로드뷰</button>
		<button class="dropdown-item" type="button" onclick="kakao_navi('<?=DEFAULT_ADDRESS.' '.$address?>','<?=$address?>');">길찾기</button>
		<button class="dropdown-item" type="button" onclick="map_view('house','<?=DEFAULT_ADDRESS.' '.$address?>')">지도보기</button>
	</div>
	<?php
	$html = ob_get_contents();
	ob_end_clean();
...
function get_all_past_records($table, $pid){
    ...
    foreach($return as $record) {
        ...
        // 새로운 방문으로 저장
        if($status === '' || $status === 'absence' || // 새로운 방문 시작 조건
           !( // 같은 방문으로 묶이는 조건의 반대
            ...
            if(!empty($current_records)) {
                ...
                foreach($current_records as $rec) {
                    ...
                    if($has_start) $has_any_start = true;
                    if($has_end) $has_any_end = true;
                }
...
function cho_hangul($str) {
	...
	if ($code > -1 && $code < 11172) { $cho_idx = $code / 588; $result .= $cho[$cho_idx]; }
	return $result;
}
