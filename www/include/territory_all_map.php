...
<?php
$sql = "SELECT tt_id, tt_num, tt_name, tt_type, tt_polygon FROM ".TERRITORY_TABLE." ORDER BY CASE tt_type WHEN '아파트' THEN 1 WHEN '격지' THEN 2 WHEN '일반' THEN 3 WHEN '빌라' THEN 4 ELSE 5 END";
...
<div class="alert alert-info mb-0 pt-1 pb-1 align-middle" style="position: absolute;bottom: 0;z-index: 9999;width: 100%;">
  <span style="display: inline-block;width: 20px;vertical-align: middle;height: 20px;margin: 7px;background-image: url(&quot;http://i1.daumcdn.net/localimg/localimages/07/mapjsapi/toolbox.png&quot;);background-position: 0px -90px;"></span> 다각형으로 표시한 구역만 보여집니다.
</div>
...
<?php while($row = $result->fetch_assoc()): if(empty($row['tt_polygon'])){ continue; } $tt_polygon = $row['tt_polygon']; ?>
...
function displayArea(area) {
  ...
  daum.maps.event.addListener(polygon, 'click', function(mouseEvent) {
    ...
    customOverlay.setContent('<div class="area">구역 번호: ' + area.tt_num + '<br>구역 이름: ' + area.tt_name + '<br>구역 형태: '+ area.tt_type +'<br>세대수: '+ area.count +'</div>');
