...
if($member_string){
  ...
  $a = 0;
  while ($mb_row = $mb_result->fetch_assoc()) $volunteered .= '<span class="badge badge'.(($mb_row['mb_id'] == mb_id())?'-success':'-light').' p-2 m-1 align-middle">'.$mb_row['mb_name'].'</span>';
}
?>
<div class="mb-3">
  <small class="badge badge-pill badge-light align-middle"><?=get_meeting_schedule_type_text($row['ms_type'])?></small>
  <?php if(!empty($g_name)) echo '<span class="badge badge-pill badge-light text-primary align-middle">집단 봉사⋮'.$g_name.'</span>'; ?>
...
<div class="mb-3">
  ...
  if(!empty($msw['m_guide'])){
    ...
    if($msw['ms_guide']) foreach (get_guide_data($msw['ms_guide']) as $value) echo '<a class="btn btn-sm btn-outline-primary m-1" href="tel:'.$value['hp'].'"><i class="bi bi-telephone"></i> '.$value['name'].'</a>';
    if($msw['ms_guide2']) foreach (get_guide_data($msw['ms_guide2']) as $value) echo '<a class="btn btn-sm btn-outline-secondary m-1" href="tel:'.$value['hp'].'"><i class="bi bi-telephone"></i> '.$value['name'].'</a>';
  }
  ?>
  <?php if(GUIDE_APPOINT_USE == 'use' && (is_admin($mb_id) || check_include_guide($mb_id, $msw['ms_guide']) )):?>
  <div class="row mt-1">
      <div class="col-7">
        <select class="form-control mr-2" name="guide"><option value="">당일 인도자 없음</option><?php echo get_guide_option($msw['m_guide']); ?></select>
      </div>
      <div class="col-5">
        <button class="btn btn-outline-primary" type="button" onclick="guide_meeting_work('appoint', '<?=$ms_id?>', '<?=$s_date?>', '<?=$page?>')">모임 임명</button>
      </div>
  </div>
  <?php endif; ?> 
</div>
...
<div class="row mb-3">
    <div class="col-7">
      <select class="form-control mb-1" name="cancle_type"><option value="1">취소 알림 노출</option><option value="2">취소 알림 비노출</option></select>
      <input type="text" name="cancle_reason" class="form-control" placeholder="취소사유">
    </div>
    <div class="col-5">
      <button class="btn btn-outline-danger" type="button" onclick="guide_meeting_work('','<?=$ms_id?>','<?=$s_date?>','<?=$page?>')">모임 취소</button>
    </div>
</div>
<?php else: ?>
  <button class="btn btn-outline-success" type="button" onclick="guide_meeting_work('0','<?=$ms_id?>','<?=$s_date?>','<?=$page?>')">모임 복원</button>
<?php endif; ?>
<?php endif; ?>