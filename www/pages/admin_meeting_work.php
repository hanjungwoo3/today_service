...
if(isset($work) && $work == 'del'){ // 모임 계획 삭제
  ...
  if(empty($ms_id)){ // 모임 계획 추가
    ...
    $m_sql = "UPDATE ".MEETING_TABLE."
              SET ms_time = '{$time}', ms_guide = '{$guide_str1}', ms_guide2 = '{$guide_str2}', mp_id = '{$place}', ms_type = '{$type}', mp_name = '".$mp['mp_name']."', mp_address = '".$mp['mp_address']."', g_id = '{$group}', m_start_time = '{$st_time}', m_finish_time = '{$fi_time}'
