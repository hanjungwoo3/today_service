...
$cancle_sql = "SELECT m.m_id
    FROM " . MEETING_TABLE . " m
    INNER JOIN " . MEETING_SCHEDULE_TABLE . " ms ON m.ms_id = ms.ms_id
    WHERE m.m_date = '{$today}'
      AND m.m_cancle = '1'
      AND (ms.ma_id IN({$ma_id}) OR ms.ma_id = '0')
      AND ms.ms_week = '{$week}'";

if (get_member_display($mb_id) == 1) {
  $cancle_sql .= " AND ms.ms_type != '2'";
}

$cancle = $mysqli->query($cancle_sql);
  ...
  <div class="list-group list-group-flush" style="margin-top: -12px">
    <?php if ($result->num_rows > 0): ?>
      <?php while ($mar = $result->fetch_assoc()): ?>
        <div class="list-group-item d-flex justify-content-between align-items-center px-3 py-2 border-bottom border-light">
          ...
          <a href="<?= BASE_PATH ?>/pages/minister_schedule.php#minister_event"
            class="btn btn-outline-secondary btn-sm badge">상세 보기</a>
        </div>
      <?php endwhile; ?>
    <?php endif; ?>
    ...
    <?php if ($cancle->num_rows > 0): ?>
      <div class="list-group-item d-flex justify-content-between align-items-center px-3 py-2 border-bottom border-light">
        ...
        <a href="javascript:void(0)" class="btn btn-outline-secondary btn-sm badge"
          onclick="open_meeting_view('<?= $today ?>')">상세보기</a>
      </div>
    <?php endif; ?>
    ...
    <?php if (board_new($mb_id)): ?>
      <div class="list-group-item d-flex justify-content-between align-items-center px-3 py-2 border-bottom border-light">
        ...
        <a href="<?= BASE_PATH ?>/pages/board.php?auth=<?= board_new($mb_id) ?>"
          class="btn btn-outline-secondary btn-sm badge">상세보기</a>
      </div>
    <?php endif; ?>
