...
$meetings = [];
$sql = "SELECT m_id, ms_id, ms_time, ms_type, mp_name, mb_id FROM t_meeting WHERE m_date = '{$date}' AND ms_type IN (1, 2) ORDER BY ms_type, ms_time";
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $meetings[] = [
            'id' => $row['m_id'],
            'ms_id' => $row['ms_id'],
            'label' => (($row['ms_type'] == 2) ? '[전시대] ' : '') . substr($row['ms_time'], 0, 5) . ' ' . $row['mp_name'] . ' (' . $count . '명)'
        ];
    }
}
