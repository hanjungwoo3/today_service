...
define('VERSION', '2026022805'); // js, css 버전
define('APP_VERSION', '2.5.13'); // 앱 버전 (대규모변경).(보이는부분&기능추가).(보이지않는부분)
...
if (get_site_option('attend_before') == '') {
  define('ATTEND_BEFORE', 45);
} else {
  define('ATTEND_BEFORE', get_site_option('attend_before'));
} // 참석버튼 노출시간 전
if (get_site_option('attend_after') == '') {
  define('ATTEND_AFTER', 15);
} else {
  define('ATTEND_AFTER', get_site_option('attend_after'));
} // 참석버튼 노출시간 후
if (get_site_option('attend_display_before') == '') {
  define('ATTEND_DISPLAY_BEFORE', 45);
} else {
  define('ATTEND_DISPLAY_BEFORE', get_site_option('attend_display_before'));
} // 전시대 참석버튼 노출시간 전
if (get_site_option('attend_display_after') == '') {
  define('ATTEND_DISPLAY_AFTER', 15);
} else {
  define('ATTEND_DISPLAY_AFTER', get_site_option('attend_display_after'));
} // 전시대 참석버튼 노출시간 후
