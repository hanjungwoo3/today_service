...
if ($result->num_rows > 0) {
  while ($row = $result->fetch_assoc()) {
    ...
    if ($row['ms_type'] == 2 && get_member_display($mb_id) == 1)
      continue;

    $m = get_meeting_data(get_meeting_id($today, $row['ms_id']));

    $attend_limit = get_meeting_limit($today, $row['ms_id']);
        ...
        <div class="list-group list-group-flush border-bottom border-light">
          <div class="list-group-item list-group-item-action d-flex flex-nowrap justify-content-between px-3 py-2">
            <div class="w-100 text-dark" onclick="open_meeting_info('<?= $today ?>','<?= $row['ms_id'] ?>','home')">
              <div>
                <span
                  class="badge badge-pill badge-light align-middle"><?= get_meeting_schedule_type_text($row['ms_type']) ?></span>
                <?php if (!empty($row['g_name']))
                  echo '<span class="badge badge-pill badge-light text-primary align-middle">집단 봉사⋮' . $row['g_name'] . '</span>'; ?>
              ...
              <div>
                <span class="align-middle"> <?= get_datetime_text($row['ms_time']) ?>         <?= $str_guide ?> <i
                    class="bi bi-info-circle text-secondary"></i></span>
              </div>
            ...
            <div class="align-self-center flex-shrink-0">
              <?php if ($status == 'attend'): ?>
                <button type="button" class="btn btn-outline-danger"
                  onclick="attend_ministry('<?= $today ?>',<?= $row['ms_id'] ?>,this);">불참</button>
              <?php else: ?>
                <?php if (!empty($attend_limit) && $count >= $attend_limit): ?>
                  <button type="button" class="btn btn-outline-primary disabled" disabled>마감</button>
                <?php else: ?>
                  <button type="button" class="btn btn-outline-primary"
                    onclick="attend_ministry('<?= $today ?>',<?= $row['ms_id'] ?>,this);">참석</button>
                <?php endif;
              endif; ?>
            </div>
          </div>
        ...
        <div class="list-group mb-2">
          <div class="list-group-item d-flex flex-nowrap justify-content-between p-2 border-light">
            <div class="flex-grow-1 pr-2">
              <div class="mb-1">
                <span class="badge badge-pill <?= $color_type ?> badge-outline px-1 align-middle"><?= $row['tt_num'] ?> ·
                  <?= get_type_text($row['tt_type']) ?></span>
              ...
              <div class="mt-n2">
                <small class="text-secondary d-inline-block">
                  <?php if ($row['tt_type'] == '편지'): ?>
                    전체 <?= $territory_progress['total'] ?> · 발송 <?= $territory_progress['visit'] ?> · 남은 집
                    <?= $territory_progress['total'] - $territory_progress['visit'] ?>
                  <?php else: ?>
                    전체 <?= $territory_progress['total'] ?> · 만남 <?= $territory_progress['visit'] ?> · 부재
                    <?= $territory_progress['absence'] ?> · 남은 집
                    <?= $territory_progress['total'] - $territory_progress['visit'] - $territory_progress['absence'] ?>
                  <?php endif; ?>
                </small>
              </div>

              <?php if ($assigned_group)
        ...
        <div class="list-group mb-2">
          <div class="list-group-item d-flex flex-nowrap justify-content-between p-2 border-light">
            <div class="flex-grow-1 pr-2">
              ...
              <div class="mt-n2">
                <small class="text-secondary d-inline-block">
                  전체 <?= $telephone_progress['total'] ?> · 만남 <?= $telephone_progress['visit'] ?> · 부재
                  <?= $telephone_progress['absence'] ?> · 남은 집
                  <?= $telephone_progress['total'] - $telephone_progress['visit'] - $telephone_progress['absence'] ?>
                </small>
              </div>

              <?php if ($tp_assigned_group)
                echo '<div class="assigned_group_name mt-1">' . $tp_assigned_group . '</div>'; ?>
            </div>
            <div class="align-self-center flex-shrink-0">
              <button type="button" class="btn btn-outline-secondary"
                onclick="open_telephone_view(<?= $tp_id ?>,'start')">시작</button>
            </div>
          </div>
        </div>
      <?php endwhile;
