...
foreach ($meeting_tp_array as $ms_id => $telephone_con) {
  ...
  if (empty($telephone_con[3]))
    $telephone_con[3] = 0;
  if (empty($telephone_con[2]))
    $telephone_con[2] = 0;
  if (empty($telephone_con[1]))
    $telephone_con[1] = 0;
  if (empty($telephone_con[0]))
    $telephone_con[0] = 0;
...
<h5 class="border-bottom mt-4 mb-3 pb-2 clearfix"><span class="align-middle mt-2 d-inline-block">전화 구역 현황</span><small
    class="mt-2 float-right">전체 구역 : <?= $week_telephone['sum'] ?>개</small></h5>
