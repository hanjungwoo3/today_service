  ...
  <div id="container" class="container-fluid">
    ...
    <nav class="navbar navbar-light bg-light mb-4">
      <a class="navbar-brand"
        href="<?= BASE_PATH ?>/pages/guide_history.php?s_date=<?= $date ?>&toYear=<?= $date_parts[0] ?>&toMonth=<?= $date_parts[1] ?>"><i
          class="bi bi-arrow-left"></i></a>
      <div class="w-75 float-right text-right mb-0 clearfix"
        onclick="open_meeting_info('<?= $date ?>','<?= $ms_id ?>','guide_assign')">
        <div>
          <small
            class="badge badge-pill badge-light align-middle"><?= get_meeting_schedule_type_text($meeting_data['ms_type']) ?></small>
          <?php
          // 취소 뱃지
        ...
        <div class="mb-0">
          <?= get_datetime_text($meeting_data['m_date'] . ' ' . $meeting_data['ms_time']) ?> <i
            class="bi bi-info-circle align-top text-secondary"></i>
        </div>
        ...
        <div class="col-12">
          <ul class="nav nav-pills nav-fill" style="flex-wrap: nowrap; font-size: 0.85rem;">
            <li class="nav-item" style="flex: 1; min-width: 0;">
              <a class="nav-link" v-on:click="changeMode('attend')" :class="mode=='attend'?'active':''"
                style="white-space: nowrap; padding: 0.5rem 0.25rem;">참여자 선택</a>
            </li>
            <li class="nav-item" style="flex: 1; min-width: 0;">
              <a class="nav-link" v-on:click="changeMode('assign')" :class="mode=='assign'?'active':''"
                style="white-space: nowrap; padding: 0.5rem 0.25rem;">구역 배정</a>
            </li>
            <?php if (GUIDE_MEETING_CONTENTS_USE == 'use'): ?>
              <li class="nav-item" v-if="auth" style="flex: 1; min-width: 0;">
                <a class="nav-link" v-on:click="changeMode('contents')" :class="mode=='contents'?'active':''"
                  style="white-space: nowrap; padding: 0.5rem 0.25rem;">모임 내용 기록</a>
              </li>
            <?php endif; ?>
    ...
    <div class="fixed-bottom container-fluid bg-white py-2 border-top" style="bottom:80px;"
      v-if="mode=='assign' && auth">
      ...
      <div class="row">
        <div class="col-12">
          ...
          <template v-else>
            <span class="badge badge-info badge-territory mr-2 my-1 bedge-big "
              :class="selected_territories.includes(value.id)?'':'d-none'" v-for="(value, index) in territories"
              v-on:click="selectTerritory(value.id,index)" v-if="value.type!='편지'">{{value.num}} · 일반</span>
            <span class="badge badge-info badge-territory mr-2 my-1 bedge-big"
              :class="selected_telephones.includes(value.id)?'':'d-none'" v-for="(value, index) in telephones"
              v-on:click="selectTelephone(value.id,index)">{{value.num}} · 전화</span>
            <span class="badge badge-info badge-territory mr-2 my-1 bedge-big"
              :class="selected_territories.includes(value.id)?'':'d-none'" v-for="(value, index) in territories"
              v-on:click="selectTerritory(value.id,index)" v-if="value.type=='편지'">{{value.num}} · 편지</span>
            <span class="badge badge-info badge-territory mr-2 my-1 bedge-big"
              :class="selected_displays.includes(value.id)?'':'d-none'" v-for="(value, index) in displays"
              v-on:click="selectDisplay(value.id,index)">{{value.name+' '+value.num+'팀'}} · 전시대</span>
      ...
      <div class="row py-2">
        <div class="col-12">
          <template v-if="guide_assigned_group_use=='use'">
            <select class="form-control float-left d-inline-block mr-2" v-on:change="changeAssignedGroup($event)"
              style="width:110px;" id="assign_group">
            ...
            <input type="text" class="form-control float-left" :class="assigned_group=='custom'?'':'d-none'"
              style="width:140px;" id="custom_assign_group" placeholder="예) 2.3.2"
              v-on:input="customAssignGroup($event)">
          </template>
          <button type="button" class="btn btn-outline-primary float-right" v-on:click="assignMember()"><i
              class="bi bi-save"></i> 배정</button>
        </div>
      </div>
    ...
    <template v-if="mode=='attend'">
      <div class="row">
        <div class="col-12">
          <h5 class="border-bottom mt-4 mb-3 pb-2">참여자<small class="float-right">{{getCountAttendMember()}}명</small>
          </h5>
          <template v-for="member in filteredMembers()">
            <button class="mb btn mb-2 mr-1 btn-member active" v-if="member.attend==1" :uid="member.mb_id"
              :class="[member.mb_sex=='W'? 'btn-outline-danger' : 'btn-outline-primary']"
              v-on:click="attendMember(member.mb_id)">{{member.mb_name}}</button>
          </template>
        </div>
      </div>
      <div class="row">
        <div class="col-12">
          ...
          <template v-for="(member, index) in filteredMembers()">
            <button class="mb btn mb-2 mr-1 btn-member" v-if="member.mb_id != 0" :uid="member.mb_id"
              :class="[member.attend==1? 'd-none' : '',member.mb_sex=='W'? 'btn-outline-danger' : 'btn-outline-primary']"
              v-on:click="attendMember(member.mb_id,index)">{{member.mb_name}}</button>
    ...
    <template v-if="mode=='assign'">
      <div class="row">
        <div class="col-12">
          <h5 class="border-bottom mt-4 mb-3 pb-2">참여자<small class="float-right">{{getCountAttendMember()}}명 중
              {{assigned_members.length}}명 배정됨</small></h5>
          <template v-for="(member, index) in filteredMembers()">
            <button class="mb btn mb-2 mr-1 btn-member position-relative" v-if="member.attend==1" :uid="member.mb_id"
              :disabled="member.attend==1? false : true"
              :class="[member.attend==1&&member.mb_sex=='W'? 'btn-outline-danger' : '',member.attend==1&&member.mb_sex=='M'? 'btn-outline-primary' : '',selected_members.includes(member.mb_id)?'active':'',assigned_members.includes(member.mb_id)?'btn-member-assigned':'']"
              v-on:click="selectMember(member.mb_id)">{{member.mb_name}}<i
                class="bi bi-bookmark-check-fill text-warning" v-if="assigned_members.includes(member.mb_id)"
                style="position: absolute;top: -6px;left: -3px;"></i></button>
          </template>
        </div>
      </div>
        ...
        <div class="col-12">
          ...
          <ul class="nav nav-tabs mb-4" id="v-pills-tab" role="tablist">
            <li class="nav-item" role="presentation">
              <a class="nav-link text-center active" id="v-pills-territory-tab" data-toggle="tab"
                href="#v-pills-territory" role="tab" aria-controls="v-pills-territory" aria-selected="true"
                v-on:click="onTabClick">일반<br><small>({{getCountAssignableTerritory()}})</small></a>
            </li>
            <li class="nav-item" role="presentation">
              <a class="nav-link text-center" id="v-pills-telephone-tab" data-toggle="tab" href="#v-pills-telephone"
                role="tab" aria-controls="v-pills-telephone" aria-selected="false"
                v-on:click="onTabClick">전화<br><small>({{getCountAssignableTelephone()}})</small></a>
            </li>
            <li class="nav-item" role="presentation">
              <a class="nav-link text-center" id="v-pills-letter-tab" data-toggle="tab" href="#v-pills-letter"
                role="tab" aria-controls="v-pills-letter" aria-selected="false"
                v-on:click="onTabClick">편지<br><small>({{getCountAssignableLetter()}})</small></a>
            </li>
            <li class="nav-item" role="presentation">
              <a class="nav-link text-center" id="v-pills-display-tab" data-toggle="tab" href="#v-pills-display"
                role="tab" aria-controls="v-pills-display" aria-selected="false"
                v-on:click="onTabClick">전시대<br><small>({{getCountAssignableDisplay()}})</small></a>
            </li>
            <li class="nav-item" role="presentation">
              <a class="nav-link text-center" id="v-pills-assign-tab" data-toggle="tab" href="#v-pills-assign"
                role="tab" aria-controls="v-pills-assign" aria-selected="true"
                v-on:click="onTabClick">배정됨<br><small>({{getCountAssignedTotal()}})</small></a>
            </li>
          </ul>

          <div class="tab-content" id="v-pills-tabContent">
            <div class="tab-pane fade show active" id="v-pills-territory" role="tabpanel"
              aria-labelledby="v-pills-territory-tab">
              <!-- 구역 형태 필터 -->
                ...
                <div class="d-flex flex-column">
                  ...
                  <div class="d-flex flex-wrap">
                    <div class="form-check form-check-inline mb-1">
                      <input type="radio" class="form-check-input" name="territoryTypeFilter" id="filterAllTypes"
                        value="all" v-model="territoryTypeFilter">
                    ...
                    <div class="form-check form-check-inline mb-1" v-for="type in territoryTypes" v-if="type!='편지'">
                      <input type="radio" class="form-check-input" :name="'territoryTypeFilter'" :id="'filterType'+type"
                        :value="type" v-model="territoryTypeFilter">
              ...
              <ul class="list-group">
                <template v-for="(value, index) in territories"
                  v-if="isTerritoryAssignable(value)&&(territoryTypeFilter == 'all' || value.type == territoryTypeFilter)">
                  <li class="list-group-item clearfix border-bottom-0 p-2" v-on:click="selectTerritory(value.id,index)"
                    ...
                    <div class="mt-n1">
                      <div class="progress d-inline-flex align-middle w-100" style="height: 5px;">
                        <div class="progress-bar" :class="(value.progress || 0) == 100 ? 'bg-success' : 'bg-warning'"
                          role="progressbar" :style="'width:'+(value.progress || 0)+'%'"
                          :aria-valuenow="value.progress || 0" aria-valuemin="0" aria-valuemax="100"></div>
                      </div>
                    </div>
                    <div class="mt-n2">
                      <small class="text-secondary d-inline-block">
                        전체 {{value.total}} · 만남 {{value.visit}} · 부재 {{value.absence}} · 남은 집 {{value.total -
                        value.visit - value.absence}}
                      </small>
                    </div>

                    <div class="mt-2 px-2 py-1 bg-light rounded"
                      v-if="hasVisitItems(value.all_past_records, 'territory')">
                      <div v-html="visitsHtml(value.all_past_records, 'territory', value.id)"></div>
                    </div>
                  </li>
                  <div class="list-group-item clearfix border-top-0 p-2 mt-0"
                    :class="selected_territories.includes(value.id)?'active':''">
                    ...
                    <button class="btn btn-sm btn-outline-secondary float-right" type="button"
                      v-on:click="map_view('territory', value.id)">지도</button>
                  </div>
                </template>
              </ul>
            </div>
            <div class="tab-pane fade" id="v-pills-telephone" role="tabpanel" aria-labelledby="v-pills-telephone-tab">
              <ul class="list-group">
                <template v-for="(value, index) in telephones" v-if="isTelephoneAssignable(value)">
                  <li class="list-group-item clearfix border-bottom-0 p-2" v-on:click="selectTelephone(value.id,index)"
                    :class="selected_telephones.includes(value.id)?'active':''">
                    <p class="mb-1">
                      <span class="badge badge-pill badge-warning badge-outline px-1 align-middle">{{value.num}} ·
                        전화</span>
                    ...
                    <div class="mt-n2">
                      <small class="text-secondary d-inline-block">
                        전체 {{value.total}} · 만남 {{value.visit}} · 부재 {{value.absence}} · 남은 집 {{value.total -
                        value.visit - value.absence}}</small>
                    </div>
                    <div class="mt-2 px-2 py-1 bg-light rounded"
                      v-if="hasVisitItems(value.all_past_records, 'telephone')">
                  ...
                  <div class="list-group-item clearfix border-top-0 p-2 mt-0"
                    :class="selected_telephones.includes(value.id)?'active':''">
                    <button class="btn btn-sm btn-outline-secondary float-right" type="button"
                      v-on:click="ViewTelephone(value.id)">보기</button>
                  </div>
                </template>
              </ul>
            </div>
              ...
              <ul class="list-group">
                <template v-for="(value, index) in territories" v-if="isLetterAssignable(value)">
                  <li class="list-group-item clearfix border-bottom-0 p-2" v-on:click="selectTerritory(value.id,index)"
                    :class="selected_territories.includes(value.id)?'active':''">
                    <p class="mb-1">
                      <span class="badge badge-pill badge-info badge-outline px-1 align-middle">{{value.num}} ·
                        {{value.type}}</span>
                    ...
                    <div class="mt-n2">
                      <small class="text-secondary d-inline-block">
                        전체 {{value.total}} · 발송 {{value.visit}} · 남은 집 {{value.total - value.visit -
                        value.absence}}</small>
                    </div>
                    <div class="mt-2 px-2 py-1 bg-light rounded"
                      v-if="hasVisitItems(value.all_past_records, 'territory')">
            ...
            <div class="tab-pane fade" id="v-pills-display" role="tabpanel" aria-labelledby="v-pills-display-tab">
              <ul class="list-group">
                <template v-for="(value, index) in displays" v-if="value.m_id != m_id">
                  <li class="list-group-item clearfix border-bottom-0 p-2" v-on:click="selectDisplay(value.id,index)"
                  ...
                  <div class="list-group-item clearfix border-top-0 p-2 mt-0"
                    ...
                    <template v-if="(value.address != '') || (value.address != '')">
                      <button type="button" class="btn btn-outline-secondary btn-sm float-right"
                        v-on:click="ViewMap(value.address)" v-if="value.address != ''">
                        지도
                      </button>
                      <button type="button" class="btn btn-outline-secondary btn-sm mr-1 float-right"
                        v-on:click="KakaoNavi(value.address)" v-if="value.address != ''">
                        길찾기
                      </button>
                      <button type="button" class="btn btn-outline-secondary btn-sm mr-1 float-right"
                        v-on:click="ViewRoadview(value.address)" v-if="value.address != ''">
                        로드뷰
                      </button>
                    </template>
                  </div>
            ...
            <div class="tab-pane fade" id="v-pills-assign" role="tabpanel" aria-labelledby="v-pills-assign-tab">
              ...
              <ul class="list-group mb-2">
                <template v-for="(value, index) in assigned_data.territory" v-if="value.type!='편지'">
                  <li class="list-group-item clearfix border-bottom-0 p-2"
                    ...
                    <div class="mt-n1">
                      <div class="progress d-inline-flex align-middle w-100" style="height: 5px;">
                        <div class="progress-bar" :class="value.progress == 100 ? 'bg-success' : 'bg-warning'"
                          role="progressbar" :style="'width:'+value.progress+'%'" :aria-valuenow="value.progress"
                  ...
                  <div class="list-group-item clearfix border-top-0 p-2 mt-0"
                    :class="selected_territories.includes(value.id)?'active':''">
                    <button class="btn btn-sm btn-outline-danger mr-1 float-left" type="button"
                      v-on:click="cancelAssign(value.id, 'territory')" v-if="auth">배정취소</button>
              ...
              <ul class="list-group mb-2">
                <li class="list-group-item clearfix disabled p-2" v-for="(value, index) in territories_record"
                  v-if="value.type!='편지'">
                  <p class="mb-1">
                    <span class="badge badge-pill badge-success badge-outline px-1 align-middle">{{value.num}} ·
                      {{value.type}}</span>
                  ...
                  <div class="mt-n2 d-none">
                    <small class="text-secondary d-inline-block">
                      전체 {{value.total}} · 만남 {{value.visit}} · 부재 {{value.absence}} · 남은 집 {{value.total - value.visit
                      - value.absence}}</small>
                  </div>
                  <div class="mt-2 px-2 py-1 bg-light rounded"
                    v-if="hasVisitItems(value.all_past_records, 'territory')">
              ...
              <ul class="list-group mb-2">
                <template v-for="(value, index) in assigned_data.telephone">
                  ...
                  <div class="list-group-item clearfix border-top-0 p-2 mt-0"
                    :class="selected_telephones.includes(value.id)?'active':''">
                    <button class="btn btn-sm btn-outline-danger mr-1 float-left" type="button"
                      v-on:click="cancelAssign(value.id, 'telephone')" v-if="auth">배정취소</button>
                    <button class="btn btn-sm btn-outline-secondary float-left" type="button"
                      v-on:click="setSelectedMembers(value.id, 'telephone', value.assigned_ids,value.assigned_group)"
                      v-if="value.assigned_ids != '' && auth">배정불러오기</button>
                    <button class="btn btn-sm btn-outline-secondary float-right" type="button"
                      v-on:click="ViewTelephone(value.id)">보기</button>
                  </div>
              ...
              <ul class="list-group">
                <template v-for="(value, index) in displays" v-if="value.m_id == m_id">
                  ...
                  <div class="list-group-item clearfix border-top-0 p-2 mt-0"
                    :class="selected_displays.includes(value.id)?'active':''">
                    <button class="btn btn-sm btn-outline-danger mr-1 float-left" type="button"
                      v-on:click="cancelAssign(value.id, 'display')" v-if="auth">배정취소</button>
                    <button class="btn btn-sm btn-outline-secondary float-left" type="button"
                    ...
                    <template v-if="(value.address != '') || (value.address != '')">
                      ...
                      <button class="btn btn-sm btn-outline-secondary ml-1 float-right" type="button"
                        v-on:click="KakaoNavi(value.address)">길찾기</button>
                      <button class="btn btn-sm btn-outline-secondary float-right" type="button"
                        v-on:click="ViewRoadview(value.address)">로드뷰</button>
                    </template>
                  </div>
                </template>
              </ul>
      ...
      <div class="row invisible">
        <div class="col-12">
          ...
          <span class="badge badge-info badge-territory mr-2 my-1 bedge-big"
            :class="selected_territories.includes(value.id)?'':'d-none'" v-for="(value, index) in territories"
            v-if="value.type!='편지'">{{value.num}} · 일반</span>
          <span class="badge badge-info badge-territory mr-2 my-1 bedge-big"
            :class="selected_telephones.includes(value.id)?'':'d-none'"
            v-for="(value, index) in telephones">{{value.num}} · 전화</span>
          <span class="badge badge-info badge-territory mr-2 my-1 bedge-big"
            :class="selected_territories.includes(value.id)?'':'d-none'" v-for="(value, index) in territories"
            v-if="value.type=='편지'">{{value.num}} · 편지</span>
          <span class="badge badge-info badge-territory mr-2 my-1 bedge-big"
            :class="selected_displays.includes(value.id)?'':'d-none'" v-for="(value, index) in displays">{{index+1}} ·
            전시대</span>
        ...
        <form id="guide_record_form">
          ...
          <div class="form-group row">
            <div class="col-12 col-md-12">
              ...
              <textarea class="form-control" name="m_contents"
                rows="12"><?= $meeting_data['m_contents'] ? $meeting_data['m_contents'] : GUIDE_MEETING_CONTENTS ?></textarea>
            </div>
          </div>
