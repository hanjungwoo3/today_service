...
<?php if (!empty($_GET['toYear']) && !empty($_GET['toMonth']) && empty($_GET['s_date']))
  exit; ?>
  ...
  <div class="list-group mb-3">
    <?php while ($mar = $ma_result->fetch_assoc()): ?>
      <div class="list-group-item d-flex justify-content-between align-items-center p-2">
        ...
        <a href="<?= BASE_PATH ?>/pages/minister_schedule.php?s_date=<?= $s_date ?>#minister_event"
          class="btn btn-outline-secondary btn-sm badge"><small>상세보기</small></a>
      </div>
    <?php endwhile; ?>
  </div>
<?php endif; ?>
...
<div class="list-group list-group-flush">
  <?php if ($result->num_rows > 0):
    while ($row = $result->fetch_assoc()) {
      ...
      if ((!empty($dsw) && $dsw['m_cancle'] == 0) || empty($dsw)) {
        ...
        if (GUIDE_APPOINT_USE == 'use') {
          ...
          if ($guide)
            $str_guide = '(' . get_member_name($guide) . ')';
        }
    ...
    if ($i == 0)
      echo '<div class="text-center align-middle p-5">마련된 공개 모임이 없습니다.</div>'; ?>
  <?php else: ?>
