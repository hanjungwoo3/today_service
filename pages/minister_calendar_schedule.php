...
<?php if ($result->num_rows > 0): ?>
  <h5 class="border-bottom mt-4 mb-3 pb-2 clearfix"><span class="align-middle mt-2 d-inline-block">모임</span><a
      onclick="open_meeting_view('<?= $s_date ?>')" class="btn btn-outline-primary btn-sm float-right"><i
        class="bi bi-info-circle"></i> 정보</a></h5>
  <div class="list-group list-group-flush mt-3 mb-3">
    ...
    while ($row = $result->fetch_assoc()):
      ...
      $attend_limit = get_meeting_limit($s_date, $row['ms_id']);
      ...
      ?>
      <div
        class="list-group-item list-group-item-action d-flex flex-nowrap justify-content-between px-1 py-2 border-bottom border-light"
        m_id="<?= $row['m_id'] ?>">
        <div class="w-100 text-dark" onclick="open_meeting_info('<?= $s_date ?>','<?= $row['ms_id'] ?>','minister')">
          <div>
            ...
            <?php if ($row['m_cancle'] && $row['m_cancle'] != 0)
              echo '<span class="badge badge-pill badge-light text-danger align-middle">취소됨</span>'; ?>
          ...
          if ((($row['ms_type'] != 2 && MINISTER_ATTEND_USE == 'use') || ($row['ms_type'] == 2 && MINISTER_DISPLAY_ATTEND_USE == 'use' && get_member_display($mb_id) != 1)) && $row['m_cancle'] == 0 && $can_attend):
            ...
            <?php if ($status == 'attend'): ?>
              <button type="button" class="btn btn-outline-danger"
                onclick="minister_work('del',<?= $row['ms_id'] ?> ,'<?= date('Y-m-d', strtotime($s_date)) ?>', this);">불참</button>
            <?php else: ?>
              <?php if (!empty($attend_limit) && $count >= $attend_limit): ?>
