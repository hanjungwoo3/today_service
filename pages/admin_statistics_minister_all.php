...
//display total
$d_sql = "SELECT count(mb_id) as total FROM " . MEMBER_TABLE . " " . $where . " AND mb_display = '0'";
$d_result = $mysqli->query($d_sql);
$display_row = $d_result->fetch_assoc();

$minister_count_array = array();

$mb_sql = "SELECT mb_id, mb_name, mb_movein_date, mb_moveout_date, mb_display FROM " . MEMBER_TABLE . " " . $where . " ORDER BY mb_name";
$mb_result = $mysqli->query($mb_sql);
while ($mow = $mb_result->fetch_assoc()) {
  if (empty_date($mow['mb_moveout_date']))
    $mow['mb_moveout_date'] = '2300-01-01';
  ...
  if ($eday >= $mow['mb_movein_date'] && $yearmonth <= $mb_moveout_date) {
    ...
    if ($mow['mb_display'] == '0') $minister_count_array[2][$minister['2count']][] = $mow['mb_name'];
...
<table class="table table-bordered mb-5">
  ...
  <tbody class="text-center">
    <?php if (!empty($minister_count_array[1])):
      ksort($minister_count_array[1]); ?>
...
<table class="table table-bordered mb-5">
  ...
  <tbody class="text-center">
    <?php if (!empty($minister_count_array[2])):
      ksort($minister_count_array[2]); ?>
      <?php foreach ($minister_count_array[2] as $key => $value): ?>
        <tr>
          <th scope="row" class="bg-light align-middle"><?= $key ?>회</th>
          <td class="align-middle">
            <div><?= count($value) ?>명</div>
            <small class="text-muted">(<?= get_percent(count($value), $display_row['total']) . '%'; ?>)</small>
          </td>
          <td class="text-left text-muted align-middle"><?= implode(", ", $value) ?></td>
        </tr>
      <?php endforeach; ?>
...
<?php if (!isset($c_meeting_schedule_type_use[3]) || $c_meeting_schedule_type_use[3] === 'use'): ?>
  ...
  <table class="table table-bordered mb-5">
    ...
    <tbody class="text-center">
      <?php if (!empty($minister_count_array[3])):
        ksort($minister_count_array[3]); ?>
...
<?php if (!isset($c_meeting_schedule_type_use[4]) || $c_meeting_schedule_type_use[4] === 'use'): ?>
  ...
  <table class="table table-bordered mb-5">
    ...
    <tbody class="text-center">
      <?php if (!empty($minister_count_array[4])):
        ksort($minister_count_array[4]); ?>
...
<?php if (!isset($c_meeting_schedule_type_use[5]) || $c_meeting_schedule_type_use[5] === 'use'): ?>
  ...
  <table class="table table-bordered mb-5">
    ...
    <tbody class="text-center">
      <?php if (!empty($minister_count_array[5])):
        ksort($minister_count_array[5]); ?>
...
<?php if (!isset($c_meeting_schedule_type_use[6]) || $c_meeting_schedule_type_use[6] === 'use'): ?>
  ...
  <table class="table table-bordered mb-5">
    ...
    <tbody class="text-center">
      <?php if (!empty($minister_count_array[6])):
        ksort($minister_count_array[6]); ?>
