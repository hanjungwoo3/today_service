...
if ($member_string) {
  ...
  while ($mb_row = $mb_result->fetch_assoc())
    $volunteered .= '<span class="badge badge' . (($mb_row['mb_id'] == mb_id()) ? '-success' : '-light') . ' p-2 m-1 align-middle"><span class="h6">' . $mb_row['mb_name'] . '</span></span>';
}
?>
...
<div id="container" class="container-fluid">
  ...
  <div class="mb-3 p-2">
    ...
    if (!empty($msw['m_guide'])) {
      foreach (get_guide_data($msw['m_guide']) as $value)
        echo '<a class="btn btn-outline-primary m-1" href="tel:' . $value['hp'] . '"><i class="bi bi-telephone"></i> ' . $value['name'] . '</a>';
    } else {
      if ($msw['ms_guide'])
      ...
      if ($msw['ms_guide2'])
        foreach (get_guide_data($msw['ms_guide2']) as $value)
          echo '<a class="btn btn-outline-secondary m-1" href="tel:' . $value['hp'] . '"><i class="bi bi-telephone"></i> ' . $value['name'] . '</a>';
    }
    ?>
  </div>
  ...
  <div class="text-right">
    <a class="btn btn-outline-secondary m-1"
      href="<?= BASE_PATH . '/pages/meeting.php?s_date=' . $s_date . '&toYear=' . date("Y", strtotime($s_date)) . '&toMonth=' . date("n", strtotime($s_date)) ?>">목록</a>
