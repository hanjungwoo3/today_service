...
while( $row = $result->fetch_assoc() ){
  ...
  if($row['mb_id'] == '0' && isset($sub_sum[$row['tt_type']])) $sub_sum[$row['tt_type']]++;
}
...
$sql = "SELECT tt_type, tt_assigned_date, tt_end_date FROM ".TERRITORY_TABLE." WHERE mb_id = '0' AND tt_type != '편지' ORDER BY FIELD(tt_type, '일반', '아파트', '빌라', '격지', '추가1', '추가2')";
$result = $mysqli->query($sql);
...
foreach ($meeting_array as $ms_id => $territory_con) {
  ...
  if(empty($territory_con[3])) $territory_con[3] = 0;
  if(empty($territory_con[2])) $territory_con[2] = 0;
  if(empty($territory_con[1])) $territory_con[1] = 0;
  if(empty($territory_con[0])) $territory_con[0] = 0;
...
<script type="text/javascript">
  ...
  function territory_type_chart() {
    ...
    data.addRows([
      <?php
      foreach ($sum as $key => $value) $sum_arr[] = '[\''.get_type_text($key).'\', '.$value.']';
...
<h5 class="border-bottom mt-4 mb-3 pb-2 clearfix"><span class="align-middle mt-2 d-inline-block">일반 구역 현황</span><small class="mt-2 float-right">전체 구역 : <?=$total?>개</small></h5>
<div id="territory_type_chart"></div>

<h5 class="border-bottom mt-4 mb-3 pb-2 clearfix"><span class="align-middle mt-2 d-inline-block">요일별 구역 형태 비율</span></h5>

<div id="week_territory_type_chart"></div>

<div class="row">
  <div class="col-lg-6">
    <h5 class="border-bottom mt-4 mb-3 pb-2 clearfix"><span class="align-middle mt-2 d-inline-block">구역 형태별 진행률</span></h5>
