      ...
      <div
        ...
        <div class="w-100 text-dark" onclick="open_meeting_info('<?= $s_date ?>','<?= $row['ms_id'] ?>','guide')">
          <div>
            ...
            <?php if (!empty($row['ma_id']) && $row['ma_id'] != '0')
              echo '<span class="badge badge-pill badge-light text-info align-middle align-middle">회중일정</span>'; ?>
          ...
          <div class="text-secondary"><small
              class="align-middle"><?= $count ?><?= (!empty($attend_limit)) ? '/' . $attend_limit : '' ?>명 참여</small></div>
        </div>
        ...
        <?php if (is_admin($mb_id) || is_guide($mb_id)): ?>
          ...
          <div class="align-self-center flex-shrink-0">
            <button type="button" class="btn btn-outline-<?= $btn_color ?>"
              onclick="pageGoPost({url:BASE_PATH+'/pages/guide_assign_step.php',vals:[['s_date','<?= $s_date ?>'],['ms_id','<?= $row['ms_id'] ?>']]});"><?= $btn_text ?></button>
          </div>
        <?php endif; ?>
      </div>
    <?php endwhile;
