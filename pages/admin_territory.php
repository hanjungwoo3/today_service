...
<div id="v_admin_territory">
  ...
  <div id="container" class="container-fluid">
    ...
    <button type="button" class="btn btn-outline-success mb-4" onclick="statistics_map_view('admin');"><i
      ...
      <div style="overflow:hidden;" class="mb-4">
        ...
        <div class="row">
          <div class="col-4 border-right">
            <div class="row">
              <div class="col-12 text-center p-3 border-bottom">
                <button type="submit"
                  onclick="javascript: jQuery('#admin_territory_assign input[name=work]').val('check_delete');"
                  class="btn btn-outline-danger"><i class="bi bi-trash"></i> 삭제</button>
              </div>
              <div class="col-12 text-center p-3">
                <button type="submit"
                  onclick="javascript: jQuery('#admin_territory_assign input[name=work]').val('check_reset');"
                  class="btn btn-outline-secondary"><i class="bi bi-recycle"></i> 리셋</button>
              </div>
            </div>
          </div>
          <div class="col-8 align-self-center">
            ...
            <div class="mt-2">모임 계획 으로 <button type="submit"
                onclick="javascript: jQuery('#admin_territory_assign input[name=work]').val('assign');"
                class="btn btn-outline-secondary">분배</button></div>
          </div>
        </div>
      </div>
      <div id="admin-territory-list">
        <div class="mb-2 clearfix">
          ...
          <button type="button" class="btn btn-outline-primary float-right"
            v-on:click="territoryWork('add','','','','','territory');">
        ...
        <div class="table-responsive" style="min-height: 350px;">
          <table class="table mb-0" style="min-width: 1400px;">
            ...
            <thead class="thead-light">
              <tr>
                <th class="text-center align-middle"><input id="all_check" type="checkbox"
                    onclick="if($(this).is(':checked')){ $('#admin-territory-list input[type=checkbox]:not(#all_check)').prop('checked', true); $('#admin-territory-list tbody tr').addClass('checked'); }else{ $('#admin-territory-list input[type=checkbox]:not(#all_check)').prop('checked', false); $('#admin-territory-list tbody tr').removeClass('checked'); }">
                </th>
              ...
              <template v-if="territories.length>0">
                <tr v-for="(value, index) in territories">
                  <td class="text-center align-middle">
                    <input type="checkbox" name="tt_id[]" :value="value.id"
                      onchange="if($(this).is(':checked')){ $(this).parent().parent().addClass('checked'); }else{$(this).parent().parent().removeClass('checked');}">
                  </td>
                  ...
                  <td class="text-center align-middle">
                    <button class="btn btn-outline-info" type="button"
                      v-on:click="territoryWork('record',value.id,'','','');"><i class="bi bi-list-task"></i>
                      ({{value.record_count}})</button>
                  </td>
                  <td class="text-center align-middle">
                    <small>
                    ...
                    <small v-if="value.status_detail.includes('진행중')"
                      class="text-warning">{{value.status_detail}}</small>
                    <small v-else-if="value.status_detail.includes('완료')"
                      class="text-success">{{value.status_detail}}</small>
                  ...
                  <td class="text-center align-middle">
                    {{value.status}}<br /><small>{{value.latest_assigned_date}}</small></td>
                  <td class="text-center align-middle">
                    <button type="button" v-on:click="memoWork('territory',value.id);"
                      class="btn btn-outline-danger mb-0 align-middle" v-if="value.memo!=''">
                      <i class="bi bi-exclamation-triangle"></i>
                    </button>
                  </td>
                  <td class="text-center align-middle">
                    <div class="dropdown">
                      <button class="btn btn-outline-secondary" type="button" :id="'ex'+value.id" data-toggle="dropdown"
                        aria-haspopup="true" aria-expanded="false">
                        <i class="bi bi-three-dots-vertical "></i>
                      </button>
                      <div class="dropdown-menu dropdown-menu-right" :aria-labelledby="'ex'+value.id">
                        <button class="dropdown-item" type="button" v-on:click="openTerritoryView(value.id);">구역
                          보기</button>
                        <button class="dropdown-item" type="button"
                          v-on:click="territoryWork('edit',value.id,value.num,'','','territory');">구역 수정</button>
                        <button class="dropdown-item" type="button"
                          v-on:click="territoryWork('copy',value.id,value.num,'','','territory');">구역 복제</button>
                        <button class="dropdown-item" type="button"
                          v-on:click="territoryWork('del',value.id,value.num,'','');">구역 삭제</button>
                        <button class="dropdown-item" type="button"
                          v-on:click="territoryWork('house',value.id,'','','');">세대 편집</button>
                        <button class="dropdown-item" type="button"
                          v-on:click="territoryWork('upload',value.id,value.num,'','');">엑셀업로드</button>
                        <button class="dropdown-item" type="button"
                          v-on:click="territoryWork('download',value.id,'','','');">엑셀다운로드</button>
                        <button class="dropdown-item" type="button"
                          v-on:click="territoryWork('reset',value.id,value.num,'','');">구역 리셋</button>
                      </div>
                    </div>
                  </td>
                </tr>
        ...
        <template v-if="territories.length>0">
          <div class="p-3"></div>
          <v-pagination v-model="page" :length="pagelength" :total-visible="7" prev-icon="bi bi-caret-left-fill"
            next-icon="bi bi-caret-right-fill" @input="onPageChange"></v-pagination>
        </template>
