...
| `include/custom_home_assignments.php` | 홈 화면 "나의 배정 특권" 섹션 (봉사인도/평일집회/공개강연/청소마이크, 역할별 라벨 구분, 월말 +1month 오버플로 수정) |
...
| `s/talk_view.php` | 공개강연 읽기 전용 뷰 (순회대회/지역대회 유형 추가, duty 그리드 레이아웃) |
| `s/talk_admin.php` | 공개강연 관리자 편집 (순회대회/지역대회 유형 추가) |
| `s/talk_api.php` | 공개강연 API (JSON 스토리지, topic_type 화이트리스트 확장) |
| `s/talk_print.php` | 공개강연 인쇄용 (순회대회/지역대회 유형 추가) |
...
| `pages/guide_assign_step.php` | +60줄 | **중간** | 탭 내비에 "봉사 짝 배정" 탭 추가 + preselect 자동선택 JS + `$ms_id` 미정의 Warning 수정 + `group_pattern` 임의지정 자동설정 + `tab=display` 전시대 탭 자동전환 |
| `m/index.php` | +200/-7줄 | **중간** | 전시대(ms_type=2) 모임/배정 추가, `getPairCount()`에 `t_display` 조회, 추천짝 다중선택(선택순서 표시), `goToAssignSelected()`, 임의지정 group_pattern 전달 |
| `m/api/meetings.php` | +4/-1줄 | 낮음 | SQL/응답에 `ms_id`, `ms_type` 필드 추가, 전시대 라벨 표시 |
...
| `pages/guide_statistics_sub.php` | -6줄 | **높음** | v2.5.15 머지 시 잔여 while 루프 제거 (Parse error 수정) |
| `core/class.core.php` | -3줄 | **중간** | UPDATE 시 변경 없으면 불필요한 Fatal error throw 제거 |
| `pages/minister_calendar_schedule.php` | +6/-6줄 | 낮음 | `$mr` null 시 빈 배열 초기화 + 배열 키 접근에 `?? ''` 추가 (PHP 8 Warning 수정) |
| `include/territory_view.php` | +2줄 | 낮음 | `$update_page`, `$update_wrap_id` 미정의 변수 기본값 초기화 (PHP 8 Warning 수정) |
| `include/meeting_info.php` | +20줄 | 낮음 | 봉사 팝업 인도자에 봉사인도 계획표(캘린더 JSON) 이름 표시 (시간대 매칭) |
| `js/territory_msg.js` | +15줄 | 낮음 | 쪽지 줄바꿈 지원 (textarea, PC Enter전송/모바일 버튼전송, 자동 높이 조절) |

#### 머지 후 수동 확인 필요 사항

1. **`config.php`** — `BASE_PATH` 계산 분기문이 upstream에서 변경되었는지 확인. `/s/`, `/c/` 경로 조건이 누락되면 하위 모듈 동작 불가
2. **`pages/guide_assign_step.php`** — 탭 내비에 "봉사 짝 배정" 탭 + preselect/group_pattern/tab JS 블록 유지 확인
3. **`m/index.php`** — 변경량이 가장 크므로 upstream 변경과 수동 비교 필요. 전시대 관련 추가(t_display 조회, 다중선택 JS) 유지 확인
...
5. **`pages/guide_statistics_sub.php` 머지 충돌 주의**: v2.5.15에서 호별구역 통계 로직이 while→foreach로 리팩토링됨. 머지 시 이전 while 루프가 잔존하면 Parse error 발생 (Unclosed `{`). 머지 후 반드시 `php -l pages/guide_statistics_sub.php`로 문법 검증.
6. **`core/class.core.php` UPDATE 메서드**: upstream에서 `affected_rows === 0`이면 Exception을 throw하는데, 동일 데이터 저장 시에도 Fatal error 발생. 해당 throw 라인 제거 필요.
7. **`pages/minister_calendar_schedule.php` PHP 8 호환**: `$mr = fetch_assoc()` 결과가 null일 때 배열 키 접근 Warning 발생. `?: []` 초기화 및 `$mr['key'] ?? ''` 처리 필요.
8. **머지 후 PHP 문법 일괄 검증**: `find . -name "*.php" -exec php -l {} \; | grep -v "No syntax errors"` 실행하여 Parse error 사전 감지.
